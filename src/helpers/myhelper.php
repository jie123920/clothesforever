...
use app\modules\cf\models\shop_models\GoodsSpec;
use app\modules\cf\models\shop_models\ShopCountryExchange;
use app\modules\cf\models\shop_models\ShopRegion as Region;
...
//         throw new Exception('请求发生错误：' . $error);
    ...
    public static function sendEmail($subject='',$to='',$html=''){
        $params = [
            ...
            "type" => "email",
            "data" => [
                "from"=>'ClothesForever<postmaster@clothesforevermail.com>',
                "subject"=>$subject,
                "to"=>$to,
                "html"=>$html
            ],
    ...
    static function payinfo($type, $id = ''){
        ...
        $url = $shoppay."/v1/game/getdata?clientip=$clientip&l=1&source=shop&appid=7";
...
//             $child['country_code'] = $country[$country_id]['country_code'];
            $region_name = $country[$country_id]['region_name'];
            $child['region_name'] = strlen($region_name)>13?substr($region_name, 0, 10).'...':$region_name;
            $result[$id] = $child;
        }
        if ($result){
            $country_id_s = array_flip(array_column($result, 'country_id', 'id'));
            $default_country_id = isset($country_id_s[$default_country_id])?$default_country_id:235;
            $default = isset($result[$exchange_rate_id])?$result[$exchange_rate_id]:$result[$country_id_s[$default_country_id]];
            unset($result[$country_id_s[$default['country_id']]]);
        }
