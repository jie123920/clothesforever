...
use app\modules\cfhome\services\Region;
use Ucenter\Ucenter;
use app\helpers\myhelper;
use app\Library\Net\IpLocation;
use app\Library\Mlog;
use app\modules\cf\models\shop_models\ShopOrder;
...
class UcenterController extends CommonController {
    ...
    public function actionRegister($email='', $password='', $adv_key = '') {
        ...
        if (isset($returnData['data']['uid'])){
            ...
            $new_user_data = [
                ...
                'platform'=>'clothesforever',
                'shop_id'=>strstr($_SERVER['HTTP_REFERER'],"shop.clothesforever.com") ? 1 : "",
                'type'=>'gw',
                'ip'=>$clientip,
                'ref'=>$http_referer,
                'device'=> $_SERVER['HTTP_USER_AGENT']
            ];
            myhelper::sendRequest($new_user_data,'POST',true,\Yii::$app->params['MY_URL']['LC_SHOP_URL'].'/shop/user-new/create');
    ...
    public function actionGetpassword(){
        ...
        if (myhelper::grepcheck($email)){
            ...
            if ($checkUser['data'] == 1) {
                ...
                $callback_url = \YII::$app->params['MY_URL']['CF'].'?email='.$email.'&code='.$code;
                $body = 'Dear '.$email . ',<br /><br />
                            You are receiving this email because you requested a new password for your ClothesForever account.
    ...
    private function createcookie($ucenter){
        ...
        setcookie('username',$auth['username'],time()+86400,'/',\YII::$app->params['COOKIE_DOMAIN']);//供手机版读取
    }
    ...
    /**
     * 解决用户中心移动端支付pay now问题
     * 2017年6月23日 下午6:21:36
     * @author liyee
     */
    public function actionUrlApp(){
        $requset = \Yii::$app->request;
        $order_id = $requset->get('id');
        $order = ShopOrder::find()->where(['id' => $order_id])->one();
        $countrycode = $order->country_code;
        $channelwayid = $order->channel_method;
        $packid = $order_id;
        $currencyid = $order->currency_id;
        $gameid = $order->project_id;
        $userid = $order->uid;
        $l = 1;
        ...
        $url = \Yii::$app->params['MY_URL']['ShopPay'].'/v1/shoppay/info?m=createurl';
